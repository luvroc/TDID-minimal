...
if [ -f "${ROOTDIR}/.env" ]; then
    export $(grep -v '^#' "${ROOTDIR}/.env" | xargs)
fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm fabric_test_orderer.example.com fabric_test_peer0.org1.example.com fabric_test_peer0.org2.example.com
...
if [ "$MODE" == "prereq" ]; then
  ...
  infoln "Launching perfected network setup..."
  
  # Set defaults for chain-DOT project - Force override (ignore network.config)
  CHANNEL_NAME="dott-channel"
  CC_NAME="didcc"
  CC_SRC_PATH="../chaincode-did"
  CC_SRC_LANGUAGE="go"
  CC_VERSION="1.0"
  CC_SEQUENCE="1"

  # Explicitly export them to ensure they are available to subprocesses/functions
  export CHANNEL_NAME
  export CC_NAME
  export CC_SRC_PATH
  ...
  # Force CA mode as it was used in successful deployment
  CRYPTO="Certificate Authorities"
  
  # Set GOPROXY for chaincode dependencies
  ...
  infoln "1. Starting Network (with CA)..."
  networkUp

  infoln "2. Creating Channel '$CHANNEL_NAME'..."
  createChannel $BFT

  infoln "3. Deploying Chaincode '$CC_NAME'..."
  deployCC
