...
echo "[P15] ensure target gateway chaincode is deployed: ${DST_CC}"
echo "[P15] deployment is expected to be done before this probe"

cd "${FABRIC_NET_DIR}"
RID="${RID:-$(date +%s)}"
TRACE="trace-p15-dual-${RID}"
TRANSFER="transfer-p15-dual-${RID}"
SESSION="session-p15-dual-${RID}"
...
echo "[P15] lock on source cc=${SRC_CC}"
...
echo "[P15] build proof on source"
...
echo "[P15] mint with proof on target cc=${DST_CC}"
...
echo "[P15] query target trace state"
...
echo "P15_FABRIC_WITHPROOF_POSITIVE: PASS"
